...
const DURATIONS = ['전체', '당일', '1박 2일', '2박 3일', '3박 이상'];
...
export default function TourPage() {
  ...
  useEffect(() => {
    productAPI.getAll({ category: 'tour' })
  ...
  const tourProducts = useMemo(() => {
    ...
    return products;
  }, [allProducts, selectedCity, selectedDuration, searchQuery, sortBy]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* 히어로 */}
      <div className="relative bg-gradient-to-br from-violet-900 via-purple-800 to-slate-900 text-white py-16 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto px-5 sm:px-6 lg:px-8 relative z-10">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h1 className="text-[40px] font-bold tracking-tight">푸꾸옥 투어 & 티켓</h1>
              <p className="text-white/70 text-sm mt-0.5">아일랜드 호핑부터 선셋 크루즈까지, 푸꾸옥의 모든 투어를 한곳에서</p>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="투어명, 지역 검색..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 pr-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/50 focus:outline-none focus:border-white/50 focus:bg-white/15"
              />
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {CITIES.map((city) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedCity === city ? 'bg-violet-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                {city}
              </button>
            ))}
            ...
            {DURATIONS.map((d) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedDuration === d ? 'bg-violet-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                {d}
              </button>
            ))}
            <div className="ml-auto">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-3 py-1.5 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-violet-400 focus:border-transparent outline-none">
      ...
      <div className="max-w-[1200px] mx-auto px-5 sm:px-6 lg:px-8 py-10">
        <p className="text-gray-600 mb-6">
          <span className="font-bold text-gray-900">{tourProducts.length}개</span>의 투어 상품
          <span className="text-violet-600 font-semibold ml-1.5">({selectedCity})</span>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl p-16 text-center shadow-sm">
            ...
            <button onClick={() => { setSelectedCity(PRIMARY_REGION); setSearchQuery(''); }} className="mt-3 px-5 py-2 bg-violet-600 text-white rounded-xl text-sm font-medium hover:bg-violet-700">
              초기화
            </button>
          </div>
        )}
        ...
        <div className="mt-10 pt-8 border-t border-gray-200">
          ...
          <div className="flex flex-wrap gap-3">
            {[
              { href: '/hotel', label: '호텔' }, { href: '/golf', label: '골프' },
              { href: '/spa', label: '스파' }, { href: '/restaurant', label: '맛집' },
              { href: '/vehicle', label: '차량' }, { href: '/guide', label: '가이드' },
            ].map((item) => (
              <Link key={item.href} href={item.href} className="px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:border-violet-400 hover:text-violet-600 transition-colors shadow-sm">
                {item.label}
              </Link>
            ))}
          </div>
